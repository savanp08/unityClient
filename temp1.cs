...
public class DashVideoPlayer2 : MonoBehaviour
{
    ...
    private IEnumerator WaitForTSeconds(double t, VideoPlayer nextPlayer){
        nextPlayer.Pause();
        yield return new WaitForSeconds((float)t);
        VideoPlayer currPlayer = (nextPlayer == videoPlayer1) ? videoPlayer2 : videoPlayer1;
        

        int videoPlayerIndex = (nextPlayer == videoPlayer1) ? 1 : 2;
        Debug.Log(segmentNumber + " Video Prepared by player " + videoPlayerIndex);
        if (nextPlayer == videoPlayer1)
        {
            skyboxMaterial.mainTexture = renderTexture1;
        }
        ...
        nextPlayer.prepareCompleted -= PrepareCompleted;
        nextPlayer.loopPointReached += OnVideoEnded;
        ++segmentNumber;
        StartCoroutine(PrepareNextSegment(nextPlayer));
        StartCoroutine(CheckIfPlayerIsPlayingAndNotify(nextPlayer));
    }

    void PrepareCompleted(VideoPlayer videoPlayer)
    {
        videoPlayer = (videoPlayer == videoPlayer1) ? videoPlayer2 : videoPlayer1;
         VideoPlayer currPlayer = videoPlayer;
         VideoPlayer nextPlayer  = (videoPlayer == videoPlayer1) ? videoPlayer2 : videoPlayer1;

        double timeRemaining = currPlayer.length - currPlayer.time;
        Debug.Log($"-->>>> Prepare Completed for segment {segmentNumber}");
        Debug.Log($"-->>>> Time remaining for curr videoPlayer to end: {timeRemaining} seconds");
        StartCoroutine(WaitForTSeconds((float)currPlayer.length - switchTimeBeforeEnd, nextPlayer));
    }
