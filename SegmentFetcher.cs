...
public class SegmentFetcher : MonoBehaviour
{
    ...
    public IEnumerator FetchSegments(List<Representation> representations, ABRAlgorithm abrAlgorithm)
    {
        ...
        while (segmentNumber < 40)
        {
            ...
            string segmentUrl = "https://a360-vr-video-streaming-7540c67a421c.herokuapp.com/playable_split/videos/" + rep.Media.Replace("$RepresentationID$", rep.Id).Replace("$Number$", segmentNumber.ToString()).Replace(".mp4", (GetPlatformCode() == 'L'? ".webm" : ".mp4"));
...
//         {
//             Representation rep = abrAlgorithm.GetBestRepresentation(representations);
//             // string segmentUrl = "https://a360-vr-video-streaming-7540c67a421c.herokuapp.com/playable_split/videos/" + rep.Media.Replace("$RepresentationID$", rep.Id).Replace("$Number$", segmentNumber.ToString());
//             string segmentUrl = "http://localhost:3000/playable_split/videos/" + rep.Media.Replace("$RepresentationID$", rep.Id).Replace("$Number$", segmentNumber.ToString());
...
//             segmentNumber++;
//             yield return new WaitForSeconds(2);  // Fetch the next segment 2 seconds after the current segment is finished fetching
//         }
//     }

//     private IEnumerator DownloadSegment(string url, int segmentNumber, System.Action<bool> callback)
//     {
//         string filePath = Path.Combine("Desktop/optiplex/pensive-PyTorch-Temp/temp/Videos", "segment" + segmentNumber + ".mp4");
...
// public class SegmentFetcher : MonoBehaviour
// {
//     private string bufferQueueFolder;
//     private List<string> bufferQueue = new List<string>();

//     public void InitializeBufferQueue()
//     {
//         bufferQueueFolder = Path.Combine(Application.persistentDataPath, "bufferQueue");
...
//     public IEnumerator FetchSegments(Representation representation)
//     {
//         int segmentNumber = 1;
//         string segmentUrl;
//         while (true)
//         {
//             segmentUrl = ResolveUrl(representation.Media.Replace("$Number$", segmentNumber.ToString("D1")), representation);
//             if (!string.IsNullOrEmpty(segmentUrl))
//             {
//                 yield return StartCoroutine(DownloadSegment(segmentUrl, segmentNumber));
//                 segmentNumber++;
//             }
...
//     private IEnumerator DownloadSegment(string url, int segmentNumber)
//     {
//         using (UnityWebRequest webRequest = UnityWebRequest.Get(url))
//         {
//             yield return webRequest.SendWebRequest();
...
//                 yield break;
//             }
//             byte[] segmentData = webRequest.downloadHandler.data;
//             string filePath = Path.Combine(bufferQueueFolder, "segment" + segmentNumber + ".mp4");
//             File.WriteAllBytes(filePath, segmentData);
//             bufferQueue.Add(filePath);
//         }
//     }

//     private string ResolveUrl(string segmentPath, Representation rep)
//     {
...
//         else
//         {
//             Uri baseUri = new Uri("http://localhost:3000/Manifest.mpd");
